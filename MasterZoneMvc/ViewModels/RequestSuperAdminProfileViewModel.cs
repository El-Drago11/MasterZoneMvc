...
{
    public class RequestSuperAdminProfileViewModel
    {
        ...
        public Error_VM ValidInformation()
        {
            ...
            if (String.IsNullOrEmpty(FirstName)) { sb.Append(Resources.SuperAdminPanel.FirstNameRequired); vm.Valid = false; }
            else if (String.IsNullOrEmpty(LastName)) { sb.Append(Resources.SuperAdminPanel.LastNameRequired); vm.Valid = false; }
        ...
        public Error_VM VaildInformationMasterzoneLogoImage()
        {
            var vm = new Error_VM();
            vm.Valid = true;
            ...
            else if (ProfileImage != null)
            {
                // Validate Uploded Image File
                bool isValidImage = true;
                string[] validImageTypes = new string[] { "image/png" };
                if (!validImageTypes.Contains(ProfileImage.ContentType))
                {
                    isValidImage = false;
                    sb.Append(Resources.ErrorMessage.OnlyPNGImageValid_ErrorMessage);
                }
                else if (ProfileImage.ContentLength > 10 * 1024 * 1024) // 10 MB
                {
                    isValidImage = false;
    ...
    public class SuperAdminDetail_VM
    {
        public long Id { get; set; }
        public long UserLoginId { get; set; }
        public string FirstName { get; set; }

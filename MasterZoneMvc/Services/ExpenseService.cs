...
namespace MasterZoneMvc.Services
{
    public class ExpenseService
    {
        ...
        public ExpenseService(MasterZoneDbContext dbContext)
        {
            db = dbContext;
            storedPorcedureRepository = new StoredProcedureRepository(db);
        }

        public JqueryDataTable_Pagination_Response_VM<Expense_Pagination_VM> GetExpenseList_Pagination(NameValueCollection httpRequestParams, Expense_Pagination_SQL_Params_VM _Params_VM)
        {
            ...
            List<Expense_Pagination_VM> lstExpenseRecords = new List<Expense_Pagination_VM>();
            ...
            lstExpenseRecords = db.Database.SqlQuery<Expense_Pagination_VM>("exec sp_ManageExpense_Pagination @id,@businessOwnerLoginId,@userLoginId,@start,@searchFilter,@pageSize,@sorting,@sortOrder,@mode", queryParams).ToList();

            recordsTotal = lstExpenseRecords.Count > 0 ? lstExpenseRecords[0].TotalRecords : 0;

            JqueryDataTable_Pagination_Response_VM<Expense_Pagination_VM> jqueryDataTableParamsViewModel = new JqueryDataTable_Pagination_Response_VM<Expense_Pagination_VM>()
            {
                draw = _Params_VM.JqueryDataTableParams.draw,
                data = lstExpenseRecords,
        ...
        public JqueryDataTable_Pagination_Response_VM<BusinessExpense_Pagination_VM> GetBusinessExpenseList_Pagination(NameValueCollection httpRequestParams, BusinessExpense_Pagination_SQL_Params_VM _Params_VM)
        {
            _Params_VM.JqueryDataTableParams.draw = (Convert.ToInt32(httpRequestParams["draw"]));
            _Params_VM.JqueryDataTableParams.start = (Convert.ToInt32(httpRequestParams["start"]));
            _Params_VM.JqueryDataTableParams.length = (Convert.ToInt32(httpRequestParams["length"])) == 0 ? 10 : (Convert.ToInt32(httpRequestParams["length"]));
            ...
            List<BusinessExpense_Pagination_VM> lstBusinessExpenseRecords = new List<BusinessExpense_Pagination_VM>();
            ...
            lstBusinessExpenseRecords = db.Database.SqlQuery<BusinessExpense_Pagination_VM>("exec sp_ManageBusinessExpense_Pagination @id,@businessOwnerLoginId,@userLoginId,@start,@searchFilter,@pageSize,@sorting,@sortOrder,@mode", queryParams).ToList();

            recordsTotal = lstBusinessExpenseRecords.Count > 0 ? lstBusinessExpenseRecords[0].TotalRecords : 0;

            JqueryDataTable_Pagination_Response_VM<BusinessExpense_Pagination_VM> jqueryDataTableParamsViewModel = new JqueryDataTable_Pagination_Response_VM<BusinessExpense_Pagination_VM>()
            {
                draw = _Params_VM.JqueryDataTableParams.draw,
                data = lstBusinessExpenseRecords,
                recordsTotal = recordsTotal,
                recordsFiltered = recordsTotal
            };
            return jqueryDataTableParamsViewModel;

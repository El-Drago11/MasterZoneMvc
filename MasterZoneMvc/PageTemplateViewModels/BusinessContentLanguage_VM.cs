...
{
    public class BusinessLanguage_VM
    {
        public long Id { get; set; }
        public long UserLoginId { get; set; }
        public string Language { get; set; }
        public HttpPostedFile LanguageIcon { get; set; }
        ...
        public Error_VM ValidInformation()
        {
            ...
            if (String.IsNullOrEmpty(Language)) { sb.Append(Resources.BusinessPanel.LanguageRequired); vm.Valid = false; }
            else if (LanguageIcon != null)
            {
                // Validate Uploded Image File
                bool isValidImage = true;
                string[] validImageTypes = new string[] { "image/jpeg", "image/png" };
                if (!validImageTypes.Contains(LanguageIcon.ContentType))
                {
                    isValidImage = false;
                    sb.Append("Invalid image type. Please select a JPEG, PNG");
                }
                else if (LanguageIcon.ContentLength > 2 * 1024 * 1024) // 10 MB
                {
                    isValidImage = false;
    ...
    public class BusinessLanguageDetail_VM
    {
        public long Id { get; set; }
        public long UserLoginId { get; set; }
        public string Language { get; set; }
    ...
    public class BusinessLanguageDetail_Pagination_SQL_Params_VM
    {
    ...
    public class BusinessLanguageDetail_Pagination_VM
    {
        public long Id { get; set; }
        public long UserLoginId { get; set; }
        public string Language { get; set; }
